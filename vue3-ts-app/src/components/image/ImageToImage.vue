<template>
    <div class="request-container">
      <div class="input-group">
        ...
        <button @click="generateImage" :disabled="!imageFile">生成</button>
      </div>
      <div class="output">
        <img v-if="previewUrl" :src="previewUrl" class="preview" />
        <img v-if="imgUrl" :src="imgUrl" class="result" />
      </div>
    </div>
  </template>
  ...
  const onFileChange = (e: Event) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        // 去掉 data:image/...;base64, 前缀，只保留纯base64
        const result = reader.result as string;
        base64Image.value = result.split(',')[1];
      };
  ...
  const generateImage = async () => {
    if (!imageFile.value || !base64Image.value) return;
